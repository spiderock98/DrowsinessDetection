...
EYE_ALARM_ON = False
IS_EYE_RECORDING = False
IS_MOUTH_RECORD = False
FOCUS = True
STATE = None
...
# log in dropbox using google account name: spiderockAPI
...
while True:
    ...
    # display time stamp on frame
    timestamp = datetime.datetime.now()
    ts = timestamp.strftime("%A %d %B %Y %I:%M:%S%p")
    cv2.putText(frame, ts, (5, 290),
    ...
    if STATE is 1:
        ...
        if (FOCUS_COUNTER == FOCUS_CONTINOUS_FRAMES):
            ...
            tt.start()
        if not FOCUS:
            cv2.putText(frame, "Tap Trung!", (300, 20),
    ...
    for rect in rects:
        ...
        STATE = type(shape)
        # if model detect driver face shape (not flick)
        ...
        avr_eye = (eye_aspect_ratio(leftEye) +
        ...
        if avr_eye < EYE_THRESH:
            ...
            if not IS_EYE_RECORDING:
                IS_EYE_RECORDING = True
                # init path
                tt = TempImage()
        ...
        if avr_mouth > MOUTH_THRESH:
            ...
            if not IS_MOUTH_RECORD:
                IS_MOUTH_RECORD = True
                # init path
                tt = TempImage()
                # create a new blank video file
                mouthOUTPUT = cv2.VideoWriter(
                    tt.path, fourcc, 10.0, (400, 300))
            # start recording each frame
